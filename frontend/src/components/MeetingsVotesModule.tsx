...
export default function MeetingsVotesModule({
  ...
  const votesStats = useMemo(() => {
    ...
    // Is Consensus reached (needs 75% for plénière or direct standard threshold)
    const approvalRate = totalVotesCast > 0 ? (pourCount / totalVotesCast) * 100 : 0;
    const isApproved = approvalRate >= activeMeeting.quorumPercentage;
    ...
    <div className="p-6 space-y-8 overflow-y-auto h-full text-slate-300">
      ...
      <div className="border-b border-white/10 pb-5">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-1 bg-black/40 border border-white/10 rounded-lg p-1 text-[11px] font-mono text-slate-400">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-b from-black/50 to-black/35 border border-white/10 rounded-2xl p-5 space-y-4 relative overflow-hidden">
            {userRole !== "ADMIN" && userRole !== "SEC_PERM" && userRole !== "RAP_CTM" ? (
              <div className="absolute inset-0 bg-[#06060c]/95 backdrop-blur-xs flex flex-col items-center justify-center p-6 text-center z-25">
                <Lock className="h-7 w-7 text-slate-500 mb-2 animate-pulse" />
                <h4 className="text-white font-bold text-xs uppercase tracking-wider">Planification verrouillée</h4>
                <p className="text-[10px] text-slate-400 mt-2 max-w-[220px] leading-relaxed">
                  Selon le manuel d'organisation, seul le <strong className="text-blue-400">Secrétaire Permanent (SG-ITP)</strong> ou le <strong className="text-amber-400">Rapporteur</strong> est habilité à initier des convocations officielles.
                </p>
                <div className="mt-3 text-[8.5px] font-mono text-slate-500 bg-white/5 px-2 py-0.5 rounded">
            ...
            <form onSubmit={handleCreateMeetingSubmit} className="space-y-3.5 text-xs">
              <div>
                <label className="block text-slate-400 mb-1 font-semibold uppercase tracking-wider text-[9px]">Intitulé officiel :</label>
                <input
                  type="text"
                  required
                  value={newTitle}
                  ...
                  className="w-full p-2.5 rounded-lg border border-white/10 bg-black/60 font-semibold text-white focus:outline-hidden focus:border-emerald-500/50"
                />
              </div>
              
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-400 mb-1 font-semibold uppercase tracking-wider text-[9px]">Organe :</label>
                  <select
                    value={newType}
                    onChange={e => setNewType(e.target.value as any)}
                    className="w-full p-2.5 rounded-lg border border-white/10 bg-black/65 text-slate-300 font-semibold focus:outline-hidden"
                  >
                ...
                <div>
                  <label className="block text-slate-400 mb-1 font-semibold uppercase tracking-wider text-[9px]">Heure d'ouverture :</label>
                  <input
                    type="time"
                    value={newTime}
                    onChange={e => setNewTime(e.target.value)}
                    className="w-full p-2 rounded-lg border border-white/10 bg-black/65 text-slate-300 focus:outline-hidden font-mono"
                  />
                </div>
              </div>

              <div>
                <label className="block text-slate-400 mb-1 font-semibold uppercase tracking-wider text-[9px]">Document réglementaire concerné :</label>
                <input
                  type="text"
                  required
                  value={newLaw}
              ...
              <div>
                <label className="block text-slate-400 mb-1 font-semibold uppercase tracking-wider text-[9px]">Raison ou ordre du jour :</label>
                <textarea
                  value={newDescription}
                  onChange={e => setNewDescription(e.target.value)}
                  placeholder="Entrez brièvement la teneur des résolutions prévues..."
                  rows={2}
                  className="w-full p-2.5 rounded-lg border border-white/10 bg-black/60 font-semibold text-slate-300 focus:outline-hidden focus:border-emerald-500/50 resize-none"
                />
              </div>
              ...
              <div className="flex items-center justify-between p-2.5 bg-black/45 border border-white/5 rounded-lg">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <span className="block font-bold text-white text-[10px]">Activer Google Meet</span>
                ...
                <input 
                  ...
                  className="h-4 w-4 rounded accent-emerald-500 text-black cursor-pointer"
                />
              </div>

              <button
                type="submit"
                className="w-full py-2.5 bg-emerald-500 text-black font-extrabold hover:bg-emerald-400 rounded-lg transition-all cursor-pointer font-sans shadow-[0_4px_12px_rgba(16,185,129,0.15)] flex items-center justify-center gap-1.5"
              >
          ...
          <div className="bg-black/35 border border-white/10 rounded-2xl p-5 space-y-3">
            ...
            <div className="space-y-2">
              {meetings.map(m => {
                ...
                let typeColor = "bg-white/5 text-slate-400";
                if (m.type === "CTM") typeColor = "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20";
                else if (m.type === "Assemblée plénière") typeColor = "bg-purple-500/10 text-purple-400 border border-purple-500/20";
                else if (m.type === "WG") typeColor = "bg-blue-500/10 text-blue-400 border border-blue-500/20";
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-3.5 rounded-xl border transition-all flex flex-col gap-2 relative ${
                      isSelected
                        ? "bg-white/[0.07] border-emerald-500/50 shadow-xs"
                  ...
                  >
                    ...
                    {m.meetActive && (
                      <div className="flex items-center gap-1 mt-1 text-[9px] text-emerald-400 font-bold font-mono">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-[#0b0c14]/90 border border-white/10 rounded-2xl shadow-2xl p-6 space-y-6 relative overflow-hidden">
            
            {/* Header / Active metadata box */}
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/10 pb-5">
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-[9.5px] uppercase tracking-widest bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 px-2 py-0.5 rounded font-bold">
                ...
                <h3 className="text-md font-extrabold text-white mt-1.5">{activeMeeting.title}</h3>
                <p className="text-xs text-slate-400 mt-1 flex items-start sm:items-center gap-1">
                  <span className="font-bold">Sujet soumis :</span> 
                  <span className="text-slate-200 font-mono text-[11px] truncate">{activeMeeting.lawSubject}</span>
                </p>
              </div>

              <div className="text-left md:text-right shrink-0 bg-black/30 p-3 rounded-xl border border-white/5 space-y-1">
            ...
            {(() => {
              ...
              if (userRole === "RAP_CTM") {
                ...
                colorClasses = "border-emerald-500/25 bg-emerald-500/5 text-emerald-400";
                missions = [
              ...
              return (
                <div className={`p-4 rounded-xl border flex flex-col gap-3 justify-between text-xs ${colorClasses}`}>
                  <div className="space-y-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="font-extrabold uppercase tracking-wide text-xs text-white">⚙️ Mission active : {title}</span>
                      <span className="text-[9.5px] font-mono font-bold bg-[#050609] border border-white/5 px-2 py-0.5 rounded-sm uppercase tracking-wider">{postes}</span>
                    </div>
                    <p className="text-slate-300 font-medium text-[11px] leading-relaxed mt-1">
                      {description}
                    </p>
                    <div className="pt-2 text-[10.5px] text-slate-400 space-y-1 bg-black/40 p-3 rounded-lg border border-white/5 mt-2">
            ...
            <div className="grid grid-cols-4 gap-1.5 bg-black/45 p-1 rounded-xl border border-white/5 text-xs font-semibold">
              <button
                ...
                {activeMeeting.meetActive ? (
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="md:col-span-2 bg-[#05060a] border border-white/10 rounded-2xl overflow-hidden relative group">
                      <div className="aspect-video bg-[#12131a] flex flex-col items-center justify-center text-center p-6 relative">
                            ...
                            return (
                              ...
                              >
                                <div className={`h-8 w-8 rounded-full flex items-center justify-center font-extrabold text-[10.5px] mb-1.5 ${
                                  isPresent ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/30" : "bg-slate-800 text-slate-500"
                                }`}>
                        ...
                        <div className="mt-6 text-center">
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="px-5 py-2.5 bg-emerald-500 hover:bg-emerald-400 text-black font-extrabold text-xs rounded-xl shadow-[0_4px_15px_rgba(16,185,129,0.3)] inline-flex items-center gap-2 transition-all cursor-pointer uppercase tracking-wider font-sans"
                          >
                      ...
                      <div className="bg-black/80 px-4 py-3 border-t border-white/10 flex items-center justify-between text-xs font-mono text-slate-400">
                        <span className="font-semibold text-[10px] text-emerald-400 tracking-wide">{activeMeeting.meetUrl}</span>
                    ...
                    <div className="bg-black/25 border border-white/5 rounded-2xl p-4 space-y-4">
                      ...
                      <div className="space-y-2.5 text-xs pt-2">
                        ...
                        <button
                          ...
                          className="w-full py-2 px-3 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/20 rounded-lg text-emerald-400 font-bold transition-all flex items-center justify-center gap-2 cursor-pointer"
                        >
                          <Share2 className="h-4 w-4 text-emerald-400" />
                  ...
                  <div className="bg-black/40 border border-dashed border-white/10 rounded-2xl p-8 text-center space-y-3">
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 rounded-xl text-xs font-bold transition-all cursor-pointer inline-block"
                    >
            ...
            {activeSubTab === "attendance" && (
              <div className="space-y-6 animate-fadeIn">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-black/35 p-4 rounded-xl border border-white/5 text-xs">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-400 mt-1 max-w-xl">
                      Pour valider une décision technique CTM au Journal Officiel, l'Article 13 stipule un quorum strict de présence d'au moins {activeMeeting.type === "CTM" ? "2/3" : "50%"} des Membres Permanents (Votants).
                    </p>
                  </div>
                ...
                {(userRole !== "ADMIN" && userRole !== "RAP_CTM" && userRole !== "SEC_PERM") && (
                  <div className="bg-emerald-950/20 border border-emerald-550/20 rounded-xl p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 animate-fadeIn">
                    <div className="space-y-1">
                      <span className="text-[10px] text-emerald-400 font-bold uppercase tracking-wide">📱 Émargement Autonome Numérique</span>
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-black font-extrabold text-[10.5px] rounded-lg transition-all shadow-md flex items-center gap-1.5 uppercase tracking-wider font-sans shrink-0 cursor-pointer"
                    >
                        ...
                        >
                          ...
                          <button
                            ...
                            className={`px-3 py-1.5 rounded-md text-[10.5px] font-bold tracking-tight transition-all cursor-pointer ${
                              isPresent 
                                ? "bg-emerald-500 text-black shadow-[0_0_10px_rgba(16,185,129,0.25)]" 
              ...
              <div className="space-y-6 animate-fadeIn">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  ...
                  <div className="bg-black/30 border border-white/5 p-3 rounded-xl font-mono text-center">
                    ...
                    <span className={`text-lg font-extrabold ${votesStats.isApproved ? "text-emerald-400" : "text-amber-500"}`}>
                ...
                <div className="bg-black/35 border border-white/5 p-4.5 rounded-xl space-y-4">
                  ...
                  <div className="space-y-3.5 text-xs font-semibold">
                    ...
                    <div className="space-y-1 block">
                      <div className="flex justify-between font-bold text-[11.5px]">
                        <span className="text-emerald-400">Pour / Approbations (OUI)</span>
                  ...
                  <div className={`p-3 rounded-lg text-[10px] font-mono text-center font-bold ${
                    votesStats.isApproved ? "bg-emerald-500/10 text-emerald-400" : "bg-amber-500/10 text-amber-500"
                  }`}>
                    {votesStats.isApproved 
                      ? "✓ SEUIL CONDUISANT À L'HOMOLOGATION ATTEINT" 
                      : `⚠️ SCRUTIN SUR LE SEUIL DE VOTE (Avoir au moins ${activeMeeting.quorumPercentage}% favorables)`}
                  </div>
                </div>
                ...
                {activeMeeting.status !== "Terminé" && (
                  <div className="space-y-4 animate-fadeIn">
                        ...
                        {!activeMeeting.attendance["sim-user"] ? (
                          ...
                          <div className="space-y-3">
                            ...
                            {activeMeeting.expertVotes["sim-user"] && (
                              <p className="text-[10px] text-emerald-400 font-mono text-right font-semibold">
                    ...
                    {userRole === "PRES_CTM" && (
                      <div className="bg-emerald-950/15 border border-emerald-500/20 rounded-xl p-4 space-y-3">
                        ...
                        <textarea
                          ...
                          rows={3}
                          className="w-full p-2.5 rounded-lg border border-white/10 bg-black/60 font-semibold text-slate-200 text-xs focus:outline-hidden focus:border-emerald-500/50 resize-none"
                        />
                ...
                {activeMeeting.status !== "Terminé" && (userRole === "RAP_CTM" || userRole === "ADMIN") ? (
                    <div className="pt-4 border-t border-white/5 flex justify-end animate-fadeIn">
                      <button
                        ...
                        className="px-5 py-2.5 bg-emerald-500 text-black font-extrabold hover:bg-emerald-400 rounded-lg transition-all cursor-pointer flex items-center gap-1.5 shadow-md"
                      >
                  ...
                  <div className="bg-emerald-500/5 border border-emerald-500/10 p-5 rounded-2xl flex items-start gap-3">
                    ...
                    <div className="text-xs">
                      <p className="font-extrabold text-emerald-400 uppercase">SCRUTIN ARCHIVÉ ET VISÉ</p>
                ...
                {activeMeeting.pvGenerated ? (
                  <div className="bg-black/35 border border-white/10 rounded-xl p-5 space-y-4">
                    <div className="flex items-center justify-between border-b border-white/5 pb-3">
                      ...
                      <span className="text-[9px] font-bold px-2 py-0.5 bg-emerald-500/10 text-emerald-400 rounded-md select-all uppercase font-mono">
                    ...
                    <div className="p-4 bg-white/[0.02] border border-white/5 rounded-lg space-y-3.5 text-xs text-slate-300 leading-relaxed">
                      <p>
                        ...
                        <br />
                        <strong className="text-emerald-400 font-mono">"{activeMeeting.lawSubject}"</strong>.
                      </p>
                      ...
                      <div className="space-y-1 block">
                        ...
                        <p>
                          ...
                          <br />
                          Verdict du Quorum requis : <strong className="text-emerald-400">{quorumStats.isQuorumReached ? "Atteint" : "Insuffisant d'office, séance d'auditions d'avis consultatif"}</strong>.
                        </p>
                      </div>
                      ...
                      <div className="space-y-1 block">
                        ...
                        <p className="mt-1 font-mono text-[11px]">
                          Taux d'approbation d'experts : <strong className="text-white">{votesStats.approvalRate}%</strong> / consensus requis {activeMeeting.quorumPercentage}% (<strong>{votesStats.isApproved ? "CONDUIT À L'HOMOLOGATION" : "HOMOLOGATION REFUSÉE"}</strong>).
                        </p>
                      </div>
                    ...
                    <div className="flex items-center gap-3 pt-2">
                      <button
                        ...
                        className="px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-black font-extrabold text-xs rounded-lg transition-all flex items-center gap-1.5 cursor-pointer shadow-md uppercase tracking-wider font-sans"
                      >
                  ...
                  <div className="bg-black/40 border border-dashed border-white/10 rounded-2xl p-8 text-center space-y-4">
                    ...
                    <button
                      type="button"
                      onClick={() => setActiveSubTab("vote")}
                      className="px-4 py-2 bg-emerald-500 text-black font-semibold rounded-lg text-xs transition-all cursor-pointer inline-flex items-center gap-1 hover:bg-emerald-400"
                    >
      ...
      <div className="bg-black/25 border border-white/10 rounded-xl p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs font-semibold">
          ...
          <div className="bg-black/30 border border-white/5 p-4 rounded-lg">
            ...
            <div className="w-full bg-white/5 h-1 rounded-full mt-2">
              <div className="bg-emerald-500 h-1 rounded-full" style={{ width: "91.4%" }}></div>
            </div>
          </div>
          <div className="bg-black/30 border border-white/5 p-4 rounded-lg">
            <span className="text-[10px] text-slate-500 uppercase tracking-tight block">Enquêtes Publiques Ouvertes</span>
            <span className="text-xl font-mono font-extrabold text-emerald-400 mt-1 block">4 Dossiers</span>
            <div className="w-full bg-white/5 h-1 rounded-full mt-2">
              <div className="bg-emerald-400 h-1 rounded-full" style={{ width: "40%" }}></div>
            </div>
          </div>
          <div className="bg-black/30 border border-white/5 p-4 rounded-lg">
        ...
        <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-white/15 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl animate-scaleUp">
            ...
            <div className="bg-[#050609] p-6 border-b border-white/10 relative text-center flex flex-col items-center">
              ...
              <div className="h-10 w-10 bg-emerald-500 text-black font-black flex items-center justify-center rounded-xl shadow-[0_0_12px_rgba(16,185,129,0.4)] text-xs">
                CNETP
              </div>
            ...
            <div className="p-8 space-y-6 bg-[#0a0a0f] font-serif text-slate-200 text-xs leading-relaxed max-h-[420px] overflow-y-auto">
              ...
              <div className="border-b border-dashed border-white/10 pb-4 text-center">
                <p className="font-sans font-extrabold text-emerald-400 text-sm tracking-wide uppercase">PROCÈS-VERBAL DE CONCERTATION TECHNIQUE ET D'ADOPTION n°{activeMeeting.id.toUpperCase()}</p>
              ...
              <div>
                ...
                <ul className="list-disc list-inside mt-1 pl-1 space-y-1">
                  <li>Pourcentage d'approbation d'experts favorables (POUR) : <strong className="text-emerald-400">{votesStats.pourPct}%</strong> ({votesStats.pourCount} votes)</li>
                ...
                <p className="mt-2 text-emerald-400 font-bold font-sans text-[10.5px]">
              ...
              <div className="border-t border-dashed border-white/10 pt-5 mt-6 flex justify-between gap-6">
                <div className="text-center font-sans shrink-0">
                  ...
                  <div className="inline-block mt-2 px-2 py-0.5 bg-emerald-500/10 border border-emerald-500/30 rounded text-[8px] text-emerald-400 uppercase font-mono font-bold">
                    Visé permanent
                  </div>
                </div>
            ...
            <div className="bg-[#050609] p-4.5 border-t border-white/10 flex justify-between gap-3 text-xs">
              <button
                ...
                className="px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-black font-extrabold rounded-lg flex items-center gap-1.5 transition-all cursor-pointer"
              >
