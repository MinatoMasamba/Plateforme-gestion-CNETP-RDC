 ...
 * Fonctionne avec Django backend
 */
    ...
    nav.innerHTML = `
        ...
        ${state.documents.map(doc => `
            <button onclick="selectDoc('${doc.id}')" 
                ...
                <span class="truncate">${doc.title || doc.code || 'Document'}</span>
            </button>
        `).join('')}
    `;
    initLucide();
...
function renderEditor(container) {
    ...
    container.innerHTML = `
        <div class="flex-1 flex flex-col p-6 overflow-hidden">
            <div class="mb-6">
                <h2 class="text-3xl font-bold text-white mb-2">${doc.title || doc.code}</h2>
                <p class="text-slate-500 text-sm">${doc.code || ''} • Dernière modification: ${doc.lastModified ? new Date(doc.lastModified).toLocaleString('fr-FR') : 'Inconnue'}</p>
            </div>
...
async function handleSaveVersion() {
    ...
    try {
        ...
        if (result.document || result.data) {
...
async function loadInitialData() {
    ...
    try {
        const [docs, collabs, experts, wgs] = await Promise.all([
            API.fetchDocs(),
            API.fetchCollaborators(),
            API.fetchExpertsList(),
            API.fetchWorkingGroups()
        ]);

        state.documents = docs || [];
        state.collaborators = collabs || [];
        state.experts = experts || [];
        state.workingGroups = wgs || [];
        state.activeCollaborator = state.collaborators[0] || state.userProfile;

        // Injector les données si Django a fourni un utilisateur
        if (window.__DJANGO_CONTEXT__ && window.__DJANGO_CONTEXT__.user) {
            state.userProfile = {
                name: window.__DJANGO_CONTEXT__.user.fullName,
                email: window.__DJANGO_CONTEXT__.user.email,
                role: "USER"
            };
            state.activeCollaborator = state.userProfile;
        }
        ...
        // Met à jour le nom d'utilisateur dans le profil
        const userNameEl = document.getElementById('user-name');
        if (userNameEl && state.userProfile) {
            userNameEl.textContent = state.userProfile.name;
        }
    } catch (err) {
        console.error("Failed to load initial data:", err);
        state.isLoading = false;
        renderViewport("Erreur de chargement des données. Vérifiez que les APIs sont accessibles.");
    }
}
