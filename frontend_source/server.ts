...
app.post("/api/documents", (req, res) => {
  const { title, code, category, description, content, ctm } = req.body;
  if (!title || !code || !category) {
    return res.status(400).json({ error: "Le titre, le code et la catégorie sont obligatoires" });
  }
  ...
  if (existing) {
    return res.status(400).json({ error: "Une norme avec ce code existe déjà" });
  }

  // Infer CTM if not supplied
  let docCtm = ctm;
  if (!docCtm) {
    if (category === "Qualité") {
      docCtm = "CTM 8 – Sciences des Matériaux, Métrologie et Valorisation Locale";
    } else if (category === "Sécurité") {
      docCtm = "CTM 3 – Bâtiment, Urbanisme et Transition Numérique";
    } else {
      docCtm = "CTM 1 – Géotechnique et Risques Naturels";
    }
  }
  ...
  const newDoc: Document = {
    ...
    updatedBy: "Prof. Masamba Édouard",
  ...
  // Initialize a first version for this document
  const firstVersion: DocumentVersion = {
    id: `v1-${id}-${Date.now()}`,
    documentId: id,
    versionNumber: 1,
    content: newDoc.content,
    author: newDoc.updatedBy,
    email: newDoc.updatedByEmail,
    timestamp: newDoc.updatedAt,
    comment: "Création initiale de la norme",
    isRollback: false
  };
