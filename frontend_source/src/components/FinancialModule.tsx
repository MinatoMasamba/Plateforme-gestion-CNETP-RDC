...
<Document xmlns="urn:iso:std:iso:20022:tech:xsd:pain.001.001.03"
  ...
  return (
    <div className={`p-6 space-y-8 overflow-y-auto h-full transition-colors duration-200 ${isLightMode ? "bg-white text-slate-800" : "text-slate-300 bg-[#06060c]"}`}>
      ...
      {/* Banner */}
      <div className={`border-b pb-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 ${isLightMode ? "border-slate-205" : "border-white/10"}`}>
        <div>
          <div className="flex items-center gap-3 mb-1">
            <HandCoins className="h-6 w-6 text-blue-500" />
            <h2 className={`text-xl font-extrabold tracking-tight uppercase ${isLightMode ? "text-slate-900" : "text-white"}`}>Portail de Gestion Financière CNETP</h2>
          </div>
          <p className={`text-xs leading-relaxed ${isLightMode ? "text-slate-550 font-semibold" : "text-slate-400"}`}>
        ...
        <div className={`flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs border ${isLightMode ? "bg-slate-50 border-slate-250 text-slate-700" : "bg-white/5 border border-white/10"}`}>
          <span className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse"></span>
          <span className={`${isLightMode ? "text-slate-500" : "text-slate-405"}`}>Poste Connecté :</span>
          <strong className={`px-2 py-0.5 rounded font-mono uppercase text-[10px] border ${isLightMode ? "text-slate-800 bg-slate-200 border-slate-300" : "text-white bg-blue-500/10 border border-blue-500/20"}`}>
            {userRole}
          </strong>
        </div>
      </div>
      ...
      {notification && (
        <div className={`p-4 rounded-xl border flex items-center gap-3 transition-all animate-fadeIn ${
          notification.type === "success" 
            ? "bg-blue-500/10 border-blue-500/30 text-blue-600 font-extrabold"
            : "bg-indigo-505/10 border-indigo-500/25 text-indigo-650 font-extrabold"
        }`}>
          <div className={`h-5.5 w-5.5 rounded-full text-center font-bold text-xs flex items-center justify-center ${isLightMode ? "bg-slate-200" : "bg-black/40"}`}>
            ℹ
          </div>
      ...
      {isFinancialAdmin ? (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
            <div className={`border rounded-xl p-4.5 transition-colors duration-200 ${isLightMode ? "bg-slate-50 border-slate-250" : "bg-black/45 border-white/10"}`}>
              <span className="text-[10px] text-slate-500 font-bold uppercase block tracking-wider">Trésor perçu (16 structures)</span>
              <span className={`text-xl font-mono font-extrabold block mt-1 ${isLightMode ? "text-slate-900" : "text-white"}`}>${totals.totalPaid.toLocaleString()} USD</span>
              <span className="text-[10px] text-blue-600 font-bold block mt-1">Cumulé sur ${totals.totalDues.toLocaleString()} USD attendus</span>
            </div>

            <div className={`border rounded-xl p-4.5 transition-colors duration-200 ${isLightMode ? "bg-slate-50 border-slate-250" : "bg-black/45 border-white/10"}`}>
              <span className="text-[10px] text-slate-500 font-bold uppercase block tracking-wider">Quote-part Restante</span>
              <span className="text-xl font-mono text-amber-600 font-extrabold block mt-1">${totals.totalOwed.toLocaleString()} USD</span>
              <span className="text-[10px] text-slate-500 font-semibold block mt-1">Sera couverte par rappels</span>
            </div>

            <div className={`border rounded-xl p-4.5 transition-colors duration-200 ${isLightMode ? "bg-slate-50 border-slate-250" : "bg-black/45 border-white/10"}`}>
              <span className="text-[10px] text-slate-500 font-bold uppercase block tracking-wider">Jetons de présence en cours</span>
              <span className="text-xl font-mono text-indigo-600 font-extrabold block mt-1">${totals.pendingJetons.toLocaleString()} USD</span>
              <span className="text-[10px] text-slate-500 font-semibold block mt-1">Estimés à partir des émargements</span>
            </div>

            <div className={`border rounded-xl p-4.5 transition-colors duration-200 ${isLightMode ? "bg-slate-50 border-slate-250" : "bg-black/45 border-white/10"}`}>
              <span className="text-[10px] text-slate-500 font-bold uppercase block tracking-wider">Liquidations en attente</span>
              <span className="text-xl font-mono text-red-600 font-extrabold block mt-1">${totals.totalPayrollPending.toLocaleString()} USD</span>
              <span className="text-[10px] text-slate-500 font-semibold block mt-1">A régler via virement ISO-20022</span>
            </div>
          </div>
            ...
            <div className="space-y-4 animate-fadeIn">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 text-xs">
                <div className="relative w-full max-w-sm">
                  <span className="absolute left-3 top-3.5 flex items-center text-slate-400">
                  ...
                  <input
                    ...
                    className={`w-full pl-9 p-2.5 rounded-lg border font-semibold focus:outline-hidden transition-colors ${
                      isLightMode 
                        ? "bg-white border-slate-300 text-slate-800 placeholder-slate-400 focus:border-blue-500" 
                        : "border-white/10 bg-black/45 text-white placeholder-slate-500 focus:border-blue-500/50"
                    }`}
                  />
                </div>

                <div className="flex items-center gap-2">
                  <span className="text-slate-500 font-bold font-mono text-[11px]">Domaine :</span>
                  <div className={`flex gap-1 p-1 rounded-lg border ${isLightMode ? "bg-slate-100 border-slate-200" : "bg-black/40 border-white/5"}`}>
                        ...
                        className={`px-2.5 py-1 rounded text-[10.5px] font-bold transition-all ${
                          filterCategory === cat
                            ? isLightMode
                              ...
                              : "bg-blue-500/10 text-blue-400 border border-blue-500/15 font-black"
                            : isLightMode
                              ? "text-slate-600 hover:text-slate-950 hover:bg-slate-205"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {filteredFees.map(fee => {
                  ...
                  return (
                    <div key={fee.id} className={`p-4 border rounded-2xl flex flex-col justify-between gap-3 text-xs transition-colors relative overflow-hidden ${
                      isLightMode 
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between items-start gap-2">
                          <div>
                            <span className={`text-[10px] uppercase tracking-tighter font-mono px-2 py-0.5 rounded border ${
                              isLightMode 
                                ? "bg-slate-200/50 text-slate-650 border-slate-250 font-bold" 
                                : "bg-white/5 text-slate-500 border-white/5"
                            }`}>{fee.category}</span>
                            <h4 className={`font-bold text-xs mt-1.5 ${isLightMode ? "text-slate-900" : "text-slate-100"}`}>{fee.name}</h4>
                          </div>

                          <span className={`text-[8.5px] px-2 py-0.5 rounded font-bold uppercase tracking-wider ${
                            fee.status === "Payé"
                              ? isLightMode
                                ? "bg-blue-50 text-blue-700 border border-blue-300 font-extrabold"
                                : "bg-blue-500/10 text-blue-400 border border-blue-500/20"
                              : fee.status === "Partiellement payé"
                              ? isLightMode
                                ? "bg-amber-50 text-amber-700 border border-amber-300 font-extrabold animate-pulse"
                        ...
                        <div className="w-full bg-white/5 h-1 rounded-full overflow-hidden mt-1">
                          <div 
                            className={`h-full rounded-full ${fee.status === 'Payé' ? 'bg-blue-500' : fee.status === 'Partiellement payé' ? 'bg-amber-400' : 'bg-red-500'}`}
                        ...
                        {rest > 0 ? (
                          <>
                            ...
                            <button
                              ...
                              className="px-2.5 py-1 bg-blue-500/15 hover:bg-blue-500/25 font-bold text-blue-400 rounded cursor-pointer"
                            >
                              Apurer solde
                            </button>
                            <button
                          ...
                          <div className="flex items-center gap-1.5 text-blue-400 font-medium font-sans">
                    ...
                    <div key={p.id} className="p-4 bg-black/35 border border-white/10 rounded-2xl flex flex-col md:flex-row justify-between md:items-center gap-4 text-xs hover:bg-[#090b14] transition-all">
                      ...
                      <div className="text-right shrink-0 flex flex-col justify-center items-end gap-2 border-t md:border-t-0 pt-3 md:pt-0 border-white/5">
                        <div>
                          ...
                          <span className="font-mono text-blue-400 font-black text-base">${totalDue} USD</span>
                        </div>

                        <div className="flex items-center gap-1.5">
                          {p.status === "Traité" ? (
                            <span className="text-[9px] font-bold px-2 py-1 bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded uppercase">
                              Liquidé (OK)
                            </span>
                          ) : (
                            <button
                              onClick={() => processExpertPayment(p.id)}
                              className="px-2.5 py-1 bg-blue-500 text-black font-extrabold text-[10.5px] hover:bg-blue-400 rounded-md uppercase"
                            >
                              Liquider
                            </button>
                          )}
                  ...
                  <div className="space-y-3.5 text-xs text-slate-400">
                    ...
                    <div className="grid grid-cols-2 gap-4 font-mono text-[10.5px] pt-2">
                      ...
                      <div className="bg-[#05060b] p-2.5 rounded-xl border border-white/5">
                        ...
                        <strong className="text-blue-400 block mt-0.5">${totals.totalPaid.toLocaleString()} USD</strong>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-3.5 text-xs text-slate-400">
                    ...
                    <div className="p-2.5 bg-white/5 rounded-xl text-[10.5px]">
                      <p className="flex justify-between text-slate-300">
                        ...
                        <strong className="text-blue-400">${(totals.totalPaid - totals.totalPayrollPending).toLocaleString()} USD</strong>
                      </p>
                    </div>
                  </div>
                </div>
        ...
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="p-5 bg-[#05060b] border border-blue-500/20 rounded-2xl space-y-4">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5 pt-2">
                  ...
                  <div className="bg-black/40 border border-white/5 rounded-xl p-4">
                    ...
                    <span className="text-xl font-mono text-blue-400 font-extrabold block mt-2">
                      ${(simulatedExpertObj.presenceCount * simulatedExpertObj.dailyRate) + simulatedExpertObj.monthlyResearchAllowance} USD
                    </span>
                    <div className="mt-1 flex items-center gap-1">
                      <span className={`w-1.5 h-1.5 rounded-full ${simulatedExpertObj.status === 'Traité' ? 'bg-blue-400' : 'bg-amber-400 animate-pulse'}`} />
                ...
                <div className="flex flex-col sm:flex-row gap-3 pt-2">
                  <button
                    ...
                    className="flex-1 py-2.5 bg-blue-500 text-black font-extrabold text-xs uppercase hover:bg-blue-400 rounded-lg flex items-center justify-center gap-2 cursor-pointer shadow-md"
                  >
          ...
          <div className="bg-[#0b0c14] border border-white/10 rounded-2xl w-full max-w-lg p-6 space-y-5 shadow-2xl animate-slideIn text-slate-300">
            ...
            <div className="flex justify-end gap-2 text-xs pt-4 border-t border-white/10 font-sans">
              ...
              <button
                ...
                className="px-4 py-2 bg-blue-500 text-black font-extrabold rounded-lg hover:bg-blue-400 cursor-pointer"
              >
                Transmettre facture
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-[#0b0c14] border border-white/10 rounded-2xl w-full max-w-md p-6 space-y-5 shadow-2xl animate-slideIn text-slate-300 font-sans">
            <div className="text-center pb-3 border-b border-white/10">
              <p className="text-[10px] text-blue-400 uppercase tracking-widest font-mono font-bold">REÇU D'EFFETS D'INDEMNITÉ</p>
            ...
            <div className="space-y-3.5 text-xs">
              ...
              <div className="p-3 bg-black/40 rounded-lg text-[10px] text-slate-400 border border-white/5 space-y-1">
                ...
                <p><strong>Statut :</strong> <span className="text-blue-400 font-bold">{payoutReceiptView.status === 'Traité' ? 'Liquidé via pain.001' : 'En attente de virement'}</span></p>
              </div>
            </div>
