...
export default function ExpertsModule({
  ...
  const ctmInfoList = [
    { code: "CTM 1", title: "Géotechnique", focus: "Reconnaissance des sols, stabilité de terrain, règles de fondations under tropical soil conditions" },
    { code: "CTM 2", title: "Ouvrages d'art", focus: "Barrages hydroélectriques, ponts suspendus, calcul mécanique des structures" },
    { code: "CTM 3", title: "Bâtiments", focus: "Habitations, immeubles administratifs et industriels, efficacité thermique" },
    { code: "CTM 4", title: "Aéroports", focus: "Harmonisation OACI, résistance des pistes d'atterrissage lourdes, tarmacs" },
    { code: "CTM 5", title: "Routes, Chemins de fer et Ports", focus: "Chaussées lourdes, plateformes ferroviaires, dalles de ports fluviaux" },
    { code: "CTM 6", title: "Ressources en eau & Ingénierie hydraulique", focus: "Infrastructures de captage, traitement de l'eau REGIDESO, agro-hydraulique" },
    { code: "CTM 7", title: "Assainissement", focus: "Gestion des fluides pluviaux urbains, réseaux d'égouts, traitement solide" },
    { code: "CTM 8", title: "Recherche, Simulation & Sciences des matériaux", focus: "Méthodes d'essais physiques avancées, simulation numérique, blocs de terre comprimée (BTC) et latérites" }
  ];
  ...
  return (
    <div className={`p-6 space-y-8 overflow-y-auto h-full transition-colors duration-250 ${
      isLightMode ? "text-slate-850 bg-slate-50/15" : "text-slate-300 bg-transparent"
    }`}>
      ...
      <div className={`border-b pb-5 ${isLightMode ? "border-slate-200" : "border-white/10"}`}>
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className={`flex rounded-lg p-0.5 text-xs font-semibold border ${
            isLightMode ? "bg-slate-200/60 border-slate-300" : "bg-black/40 border-white/10"
          }`}>
            <button
              onClick={() => setExpertSelectionType("wg")}
              className={`px-3 py-1.5 rounded-md transition-all cursor-pointer ${
                expertSelectionType === "wg" 
                  ? (isLightMode ? "bg-white text-blue-800 font-bold shadow-xs border border-slate-300" : "bg-blue-500/10 text-blue-400 font-bold border border-blue-500/20")
        ...
        <p className={`text-xs max-w-3xl leading-relaxed mt-2 ${isLightMode ? "text-slate-600" : "text-slate-400"}`}>
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-black/30 border border-white/10 rounded-xl p-5 relative overflow-hidden backdrop-blur-md">
                ...
                <div className="flex items-center gap-2 mb-2 font-mono text-[10.5px]">
                  <span className="bg-blue-500/15 border border-blue-500/30 text-blue-400 px-2 py-0.5 rounded-md font-bold text-xs">
                    {activeWg.code}
                  </span>
                  <span className="text-slate-500">•</span>
                  <span className="text-slate-400 font-bold">STATUT LOCAL :</span>
                  <span className="text-blue-400 font-bold uppercase">{activeWg.tag}</span>
                </div>
                  ...
                  <button
                    ...
                    className={`p-4 rounded-xl border text-left transition-all relative overflow-hidden flex flex-col justify-between cursor-pointer ${
                      selectedNormFilter === "ALL"
                        ? "bg-blue-500/10 border-blue-500/35 text-white shadow-[0_4px_20px_rgba(16,185,129,0.08)]"
                  ...
                  >
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="scale-90 font-sans text-[10px] font-bold text-blue-400 bg-blue-500/15 px-1.5 py-0.5 rounded-full">
                    ...
                    return (
                      ...
                      >
                        <div>
                          ...
                          <h4 className="text-xs font-extrabold line-clamp-1 group-hover:text-blue-400 transition-colors">
                            {norm.title}
                          </h4>
                        ...
                        <div className="mt-4">
                          <div className="flex items-center justify-between text-[10px] text-slate-400 font-bold mb-1">
                            ...
                            <span className="font-mono text-blue-400">{norm.avgProgress}%</span>
                          </div>
                          <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden">
                            <div
                              className={`h-full rounded-full transition-all duration-500 ${norm.avgProgress === 100 ? "bg-blue-500" : "bg-amber-500"}`}
                            ...
                            <div className="space-y-2">
                              {tasksForThisNorm.map((task) => (
                                <div 
                                  ...
                                  className="bg-black/30 border border-white/5 hover:border-blue-500/25 p-3.5 rounded-lg transition-all flex items-start gap-3.5 cursor-pointer hover:bg-white/[0.02]"
                                >
                                  <div className="mt-0.5">
                                    {task.status === "Completed" ? (
                                      <div className="h-4 w-4 rounded-full bg-blue-500 flex items-center justify-center text-black font-extrabold text-[10px]">
                                        ✓
                                      </div>
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <div className="flex items-center justify-between gap-2">
                                      ...
                                      <span className={`text-[9px] px-1.5 py-0.5 font-bold rounded shrink-0 ${
                                        task.status === "Completed" ? "bg-blue-500/10 text-blue-400" : task.status === "In Progress" ? "bg-amber-500/10 text-amber-400" : "bg-white/5 text-slate-400"
                                      }`}>
                                        {task.status}
                                      </span>
                                    </div>
                                    ...
                                    <div className="mt-2.5 w-full h-1 bg-white/5 rounded-full overflow-hidden">
                                      <div 
                                        className={`h-full rounded-full transition-all duration-500 ${task.status === "Completed" ? "bg-blue-500" : task.status === "In Progress" ? "bg-amber-500" : "bg-slate-600"}`}
                  ...
                  {experts.filter(exp => exp.wg && exp.wg.includes(activeWg.code)).length === 0 ? (
                    ...
                    experts.filter(exp => exp.wg && exp.wg.includes(activeWg.code)).map((exp) => (
                      <div 
                        ...
                        className="p-3 bg-black/45 border border-white/5 rounded-lg hover:border-blue-500/30 transition-all cursor-pointer flex items-center gap-3.5 group"
                      >
                        <div className="h-9 w-9 bg-blue-500/10 border border-blue-500/20 text-blue-400 rounded-lg flex items-center justify-center font-extrabold text-xs">
                          {exp.name.split(" ").map((n: string) => n[0]).join("")}
                        </div>
                        <div className="min-w-0 flex-1">
                          <p className="text-xs font-bold text-white group-hover:text-blue-400 transition-colors truncate">{exp.name}</p>
              ...
              <div className="bg-black/30 border border-white/10 rounded-xl p-5 relative overflow-hidden backdrop-blur-md">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    <div className="h-14 w-14 rounded-full bg-blue-500 text-black flex items-center justify-center font-extrabold text-lg shadow-[0_0_20px_rgba(16,185,129,0.3)] border-2 border-blue-400">
                  ...
                  <div>
                    {activeExpert.isApproved ? (
                      <span className="text-[10px] font-bold bg-blue-500/10 text-blue-400 border border-blue-500/20 px-3 py-1.5 rounded-lg flex items-center gap-1.5 uppercase">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-white/5 pt-5 mt-5 text-xs text-slate-300">
                  ...
                  <div className="space-y-2.5">
                    <div className="flex items-center gap-2">
                      <Award className="h-4 w-4 text-blue-500/60 shrink-0" />
                      <span className="text-slate-500 font-bold">Comité CTM :</span>
                      <span className="text-blue-400 font-bold truncate max-w-[200px]">{activeExpert.ctm}</span>
                    </div>
                    <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-3 pt-2">
                    {!activeExpert.isApproved ? (
                      <button
                        ...
                        className="px-4.5 py-2.5 rounded-lg bg-blue-500 hover:bg-blue-400 text-black font-extrabold text-xs flex items-center gap-2 transition-all cursor-pointer shadow-[0_0_12px_rgba(16,185,129,0.3)]"
                      >
                        <ShieldCheck className="h-4 w-4" />
                        Valider et Accréditer l'Expert
                      </button>
                    ) : (
                      <div className="p-3.5 bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs font-bold rounded-lg flex items-center gap-2 w-full">
              ...
              <div className="bg-black/25 border border-white/10 rounded-xl p-5 space-y-4.5">
                ...
                <div className="space-y-3 font-mono text-xs">
                 ...
                 <button
                  ...
                  className="w-full py-2.5 rounded-lg bg-blue-500/10 hover:bg-blue-500/20 border border-blue-500/30 text-blue-400 font-extrabold text-xs transition-all cursor-pointer block text-center"
                >
      ...
      <div className="bg-black/25 border border-white/10 rounded-xl p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-black/30 border border-white/5 p-4 rounded-lg">
            ...
            <div className="w-full bg-white/5 h-1 rounded-full mt-2">
              <div className="bg-blue-500 h-1 rounded-full" style={{ width: `${(experts.filter(e => e.isApproved).length / experts.length) * 100}%` }}></div>
            </div>
          </div>
          <div className="bg-black/30 border border-white/5 p-4 rounded-lg">
            <span className="text-[10px] text-slate-500 uppercase tracking-tight block">Taux d'Avancement des Normes</span>
            <span className="text-xl font-mono font-extrabold text-blue-400 mt-1 block">84.2 %</span>
            <div className="w-full bg-white/5 h-1 rounded-full mt-2">
              <div className="bg-blue-400 h-1 rounded-full" style={{ width: "84.2%" }}></div>
            </div>
          </div>
        </div>
      </div>
