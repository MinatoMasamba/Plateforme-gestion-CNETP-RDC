...
import CreateNormModal from "./components/CreateNormModal";
...
export function WorkspaceContainer() {
  ...
  const CNETP_ROLES = [
    ...
    { id: "PRES_CTM", name: "Président du CTM", desc: "Arbitrage & validations scientifiques", badgeColor: "bg-blue-500/10 text-blue-400 border-blue-500/20 hover:bg-blue-500/20" },
  ...
  // Norm-specific voting records to view stats & cast interactive votes directly in editor
  const [normVotes, setNormVotes] = useState<Record<string, Record<string, "pour" | "contre" | "abstention">>>({
    "iso-9001": {
      "exp-1": "pour",
      "exp-2": "pour",
      "exp-3": "pour",
  ...
  const handleCastNormVote = (docId: string, choice: "pour" | "contre" | "abstention") => {
    // Determine which expert profile masamba is. We know Masamba is exp-1.
    const activeExpert = experts.find(e => e.email === testProfile.email) || { id: "exp-1" };
    const expertId = activeExpert.id;

    setNormVotes(prev => {
      const docV = { ...(prev[docId] || {}) };
      docV[expertId] = choice;
      return {
        ...prev,
        [docId]: docV
      };
    });
    showToast(`Votre vote (${choice.toUpperCase()}) pour ce projet de norme a été enregistré !`, "success");
  };
  ...
  if (isLoading) {
    return (
      <div className="h-screen w-screen bg-[#020204] flex flex-col items-center justify-center gap-4">
        <Loader className="h-9 w-9 text-blue-500 animate-spin" />
  ...
  const workspaceBg = isDarkMode 
    ? "bg-[#020204] text-slate-300" 
    : "bg-slate-50 text-slate-800";

  const tabBtnClass = (active: boolean) => {
    if (active) {
      return isDarkMode
        ? "border-blue-500 text-blue-400 bg-blue-500/10 shadow-[0_4px_15px_rgba(59,130,246,0.1)] font-bold"
        : "border-blue-500 text-blue-700 bg-blue-100/70 shadow-xs font-extrabold";
    }
    return isDarkMode
  ...
  return (
    <div className={`h-screen w-screen overflow-hidden flex font-sans transition-colors duration-200 ${workspaceBg}`}>
      ...
      {toast && (
        <div className="fixed bottom-6 right-6 z-50 flex items-center gap-3 px-4.5 py-3.5 rounded-xl shadow-xl border animate-slideIn bg-[#020204]/90 backdrop-blur-md border-white/10">
          {toast.type === "success" && (
            <CheckCircle2 className="h-5 w-5 text-blue-400 shadow-[0_0_10px_rgba(16,185,129,0.3)]" />
          )}
          {toast.type === "error" && (
      ...
      <Sidebar
        ...
        onAddNewDoc={() => setIsCreateModalOpen(true)}
      ...
      <main className={`flex-1 flex flex-col h-full min-w-0 relative transition-colors duration-250 ${
        isDarkMode ? "bg-[#06060c]/20" : "bg-white border-l border-slate-200"
      }`}>
        ...
        <nav className={`h-14 px-6 flex items-center justify-between shrink-0 z-10 overflow-x-auto whitespace-nowrap scrollbar-none transition-colors duration-250 ${
          isDarkMode 
            ? "border-b border-white/10 bg-black/40 backdrop-blur-md" 
            : "border-b border-slate-200 bg-slate-50 shadow-xs"
        }`}>
          <div className="flex items-center gap-1 flex-row flex-nowrap shrink-0">
            <button
              id="nav-tab-editor"
              onClick={() => setActiveTab("editor")}
              className={`h-14 px-3 text-[10.5px] font-bold tracking-wider uppercase transition-all flex items-center gap-1.5 border-b-2 shrink-0 ${tabBtnClass(activeTab === "editor")}`}
            >
            ...
            <button
              ...
              className={`h-14 px-3.5 text-[10.5px] font-bold tracking-wider uppercase transition-all flex items-center gap-2 border-b-2 shrink-0 ${tabBtnClass(activeTab === "history")}`}
            >
            ...
            <button
              ...
              className={`h-14 px-3.5 text-[10.5px] font-bold tracking-wider uppercase transition-all flex items-center gap-2 border-b-2 shrink-0 ${tabBtnClass(activeTab === "experts")}`}
            >
            ...
            <button
              ...
              className={`h-14 px-3.5 text-[10.5px] font-bold tracking-wider uppercase transition-all flex items-center gap-2 border-b-2 shrink-0 ${tabBtnClass(activeTab === "meetings")}`}
            >
            ...
            <button
              ...
              className={`h-14 px-3.5 text-[10.5px] font-bold tracking-wider uppercase transition-all flex items-center gap-2 border-b-2 shrink-0 ${tabBtnClass(activeTab === "financial")}`}
            >
            ...
            <button
              ...
              className={`h-14 px-3.5 text-[10.5px] font-bold tracking-wider uppercase transition-all flex items-center gap-2 border-b-2 shrink-0 ${tabBtnClass(activeTab === "validation")}`}
            >
            ...
            <button
              ...
              className={`h-14 px-3.5 text-[10.5px] font-bold tracking-wider uppercase transition-all flex items-center gap-2 border-b-2 border-transparent shrink-0 cursor-pointer ${
                isDarkMode ? "text-slate-400 hover:text-white hover:bg-white/5" : "text-slate-650 hover:text-slate-950 hover:bg-slate-100"
              }`}
            >
              <BookOpen className="h-4 w-4 text-teal-500 animate-pulse" />
            ...
            {['LEGISTE', 'COORD_CTC', 'ADMIN'].includes(userRole) && (
              <button
                ...
                className={`h-14 px-3.5 text-[10.5px] font-bold tracking-wider uppercase transition-all flex items-center gap-2 border-b-2 shrink-0 ${tabBtnClass(activeTab === "legistique")}`}
              >
          ...
          {/* Theme Switcher & Simulation Role Container */}
          <div className="shrink-0 flex items-center gap-3 pr-3">
            {/* Theme Toggle Button */}
            <button
              id="theme-toggle-btn"
              onClick={() => setIsDarkMode(!isDarkMode)}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border font-bold text-xs cursor-pointer transition-all duration-200 ${
                isDarkMode 
                  ? "border-white/10 bg-white/5 text-slate-300 hover:bg-white/10 hover:text-white" 
                  : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50 hover:text-slate-900 shadow-xs"
              }`}
              title={isDarkMode ? "Passer au thème d'État clair" : "Passer au thème d'État sombre"}
            >
              <span>{isDarkMode ? "☀️ Thème Clair" : "🌙 Thème Sombre"}</span>
            </button>

            <button
              id="role-switcher-btn"
              onClick={() => setIsProfileModalOpen(true)}
              className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border font-bold text-xs cursor-pointer transition-all duration-200 ${
                isDarkMode 
                  ? "border-blue-500/30 bg-blue-500/5 hover:bg-blue-500/15 text-blue-400 hover:text-blue-350 shadow-[0_0_15px_rgba(59,130,246,0.06)]"
                  : "border-blue-300 bg-blue-50 hover:bg-blue-100 text-blue-800 shadow-xs"
              }`}
              title="Modifier mon profil d'expert et choisir un rôle de test"
            >
              <Users className={`h-3.5 w-3.5 animate-pulse ${isDarkMode ? "text-blue-500" : "text-blue-600"}`} />
              <span className={isDarkMode ? "text-slate-300" : "text-slate-600"}>Profil de test (</span>
              <span className={`font-extrabold max-w-[110px] truncate ${isDarkMode ? "text-white" : "text-slate-900"}`}>
                {testProfile.name}
              </span>
              <span className={isDarkMode ? "text-slate-300" : "text-slate-600"}>) :</span>
              <span className={`text-[10px] font-mono tracking-wide px-2 py-0.5 rounded border uppercase leading-none ${
                isDarkMode 
        ...
        <div className="flex-1 min-h-0 flex flex-col z-10">
          {activeTab === "experts" ? (
            ...
            <ValidationPublicModule isDarkMode={isDarkMode} />
          ) : activeTab === "legistique" ? (
            <LegistiqueModule userRole={userRole} isDarkMode={isDarkMode} />
            ...
            activeTab === "editor" ? (
              <EditorArea
                ...
                isDarkMode={isDarkMode}
                documentVotes={normVotes[activeDocument.id] || {}}
                onCastVote={(choice) => handleCastNormVote(activeDocument.id, choice)}
                experts={experts}
              />
            ) : (
              <HistoryArea
      ...
      {/* State-of-the-art Norm Creation Wizard Modal (Upload/Edit) */}
      <CreateNormModal
        ...
        onSuccess={(newDoc) => {
          ...
          setActiveTab("editor");
          showToast(`Avant-projet de norme ${newDoc.code} créé avec succès !`, "success");
        }}
        isDarkMode={isDarkMode}
      />
    </div>
