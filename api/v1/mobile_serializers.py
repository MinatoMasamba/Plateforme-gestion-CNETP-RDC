...
class ExpertActivationSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        # Créer NotificationPreference s'il n'existe pas
...
class MobileLoginSerializer(serializers.Serializer):
    """Login pour mobile (email + password → JWT token)"""
    email = serializers.EmailField()
...
class MobileUserDetailSerializer(serializers.ModelSerializer):
    ...
    def get_user_type(self, obj):
        if obj.is_expert:
            return 'expert'
        elif hasattr(obj, 'public_profile'):
            return 'public'
        return 'unknown'
...
class NotificationDetailSerializer(serializers.ModelSerializer):
    ...
    def get_logs(self, obj):
        logs = obj.logs.all()[:5]  # Les 5 derniers logs
